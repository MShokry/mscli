...
import { Dimensions, Image, SafeAreaView, View, Text, Platform, KeyboardAvoidingView } from 'react-native';
import { Button, Caption, HelperText, Surface, TextInput, Title, useTheme, } from 'react-native-paper';
import api from '../../utils/APICONST';
import MainContext from '../../Context/mainContext';
import formReducer from '../../Context/FormReducer';
import { logUser, logUserFB, logUserGoogle, logUserApple } from '../../utils/UsersAPI';
...
const SignIn = ({ navigation }) => {
  ...
  const pos = options.map((e) => { return e.value; }).indexOf(contextState.Lang);
      ...
      <View
        ...
        <SafeAreaView>
          <KeyboardAwareScrollView
            ...
            <View style={{ width: 73, alignSelf: 'flex-end', marginBottom: 20, marginTop: 10 }}>
              <SwitchSelector
                options={options}
                value={pos}
                initial={pos}
            ...
            <Title style={{ marginVertical: 30, fontSize: 24, color: 'black' }}>{T('loginScreen.title')}</Title>
            <TextInput
              mode="outlined"
            ...
            />
            {User.error ?
              <HelperText visible={(User.error || '').length} style={AppStyle.headLine}>
                {User.error}
              </HelperText> : null}
            <Button
              mode="contained"
              style={{ height: 47, marginVertical: 10, justifyContent: 'center' }}
              onPress={() => logUser([state, User, setUser])}
              loading={User.loading}
              dark
              disabled={User.loading}>{T('loginScreen.login_button')}</Button>

            <Button
              compact
              uppercase={false}
              mode="text"
              labelStyle={[AppStyle.headLine, { color: colors.error, marginTop: 15 }]}
              onPress={() => navigation.push('Forget')}>{T('loginScreen.forget_password')}</Button>
